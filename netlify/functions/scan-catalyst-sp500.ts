// Scheduled trigger for the sp500 catalyst scan.
//
// Cron: `0,30 13-21 * * 1-5` (every 30 min during market hours, weekdays).
// Kept at the original sp500 slot.
//
// Previously this file ran the scan inline via `runCatalystScan`. Catalyst
// is the heaviest board (4 providers/ticker); after PR #66 expanded sp500
// 208→503 the single-pass scan exceeded Netlify's 15-min background ceiling
// and wrote no snapshot from 2026-06-03 onward. It now adopts the same
// checkpoint-resume pattern as the russell2k insider/target scans: a thin
// scheduled trigger fires a background worker that batches the universe,
// checkpoints a cursor in Firestore, and self-reinvokes via
// `Context.waitUntil` until the sweep completes.
//
// The actual scan lives in `scan-catalyst-sp500-background.ts`. The trigger
...
const RUN_ID_PREFIX = 'catalyst-sp500-';

export const handler = schedule('0,30 13-21 * * 1-5', async () => {
  const log = logger.child({
    fn: 'scan-catalyst-sp500',
    universe: 'sp500',
