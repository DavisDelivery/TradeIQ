// Scheduled trigger for the lynch dow scan.
//
// Cron: 0 22 * * 1-5. The scan itself runs in scan-lynch-dow-background (Netlify grants
...
const WORKER_PATH = '/.netlify/functions/scan-lynch-dow-background';

export const handler = schedule('0 22 * * 1-5', async () => {
  const log = logger.child({ fn: 'scan-lynch-dow', universe: 'dow' });
  ...
  try {
    ...
    return { statusCode: 200, body: JSON.stringify({ ok: true, board: 'lynch', universe: 'dow', workerStatus: res.status }) };
  } catch (err: any) {
    log.error('worker_dispatch_failed', { err: String(err?.message ?? err) });
    return { statusCode: 500, body: JSON.stringify({ ok: false, board: 'lynch', universe: 'dow', error: String(err?.message ?? err) }) };
  }
});
