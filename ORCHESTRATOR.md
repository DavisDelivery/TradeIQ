...
- **`briefs/phase-4f-brief.md`** — Stub-analyst audit + repair + institutional-flow data integration. Target `0.18.0-alpha`, MODEL_VERSION bumps to `2026.03.0`. **Now the priority — Phase 5a is data-blocked; 4f is unblocked and provides direct product value.** Kickoff at `kickoffs/phase-4f-executor.md` is paste-and-go.
- **`briefs/phase-4e-1-brief.md`** — Prophet Portfolio engine + backtest validation (4e split). PR #21 merged (engine dormant); 4e-1-finish row tracks pending live-data verdict run.

### Briefs blocked / shelved
- **`briefs/phase-5a-brief.md`** — Phase 5a scaffolding shipped (PR #24 OPEN draft) but data gate cannot be cleared with current PIT coverage. See 5a row in phase table for full unblock paths. Recommendation 2026-05-14: shelve until either Phase 0a-2 (sp500 PIT backfill) ships, or several weeks of natural snapshot accumulation provide a base to revisit.

### Pending no-brief phases (capturing scope for future drafting)
- **Phase 0a-2 — PIT universe-history backfill** (no brief yet). Load-bearing unblock for 5a and every other historical-data analysis. Days of work; static-source ingestion (Wikipedia historical sp500 + Polygon ticker activity) into `universeHistory/sp500/{date}` collection. Defer brief drafting until 4f lands.
...
Recommended order: **4f next** → 4e-1-finish (live-data verdict run, separate trusted env) → revisit 5a with 0a-2 prerequisite OR rescope to Dow-pilot.
...
| 0a-2 | PIT universe-history backfill (sp500 → ndx → russell2k) | pending (no brief yet) | — | — | Discovered 2026-05-14 trying to clear Phase 5a's seed-run data gate. The `universeHistory/{universe}/{date}` collection only has live coverage starting ~2026-05-07; historical backtests with 2018-2024 windows on sp500 or ndx complete in 5 seconds with 0 trades because every rebalance date emits "universe pool empty (no PIT snapshot covers date)". Probe of russell2k ran 36+ min and never completed (treated as dead). Only `dow` has working historical PIT coverage. Phase 0a-2 scope: populate `universeHistory/sp500/{date}` from a static source (Wikipedia historical sp500 membership tables + Polygon historical ticker activity), back to 2018, survivorship-corrected via add/drop event dates. Days of work; unblocks every backtest that needs sp500 history, including Phase 5a (data gate would then clear with ~2 sp500 runs) and any retrospective Prophet/Target board scoring against historical regimes. ndx + russell2k backfill as follow-up sub-phases (0a-2b, 0a-2c). |
...
| 4f | Stub-analyst audit + repair + institutional-flow data | pending (brief ready) | — | — | Surfaced 2026-05-13 from Chad's screenshot of the Target Board ON ticker: 5 of 10 analyst contributions (Insider 14%, Political 10%, Macro 7%, Earnings 7%, Patents 6% — total 44% weight) were returning exactly 50, the neutral midpoint. Phase 4f's brief at `briefs/phase-4f-brief.md` covers BOTH the audit/repair across Target + Prophet AND new institutional-flow data integration (Polygon dark-pool prints, unusual options activity, block trades) into the surviving Flow + Insider analysts. Target version `0.18.0-alpha`, MODEL_VERSION bumps to `2026.03.0` on merge (composite weights change). Runs AFTER 4e-1 + 5a land; depends on 4e-1's W0 audit table as starting Prophet-side input. |
