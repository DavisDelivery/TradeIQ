...
| 4h | Russell scan reliability + nightly schedule + company info display | executor PR open — live acceptance deferred | 0.18.5-alpha | 2026-05-17 | Three defects fixed in one PR. **W1 checkpoint-resume:** `scan-target-board-russell2k-background.ts` + `scan-target-board-sp500-background.ts` chain across the 15-min ceiling via `shared/scan-resume/cursor.ts` + reused `backtest-resume/{watchdog,reinvoke}.ts`. Cursor at `scanRuns/{runId}` records `nextTickerIndex/totalTickers/invocationCount`; partial Target rows accumulate in `scanRuns/{runId}/partial/{batch-NNNNNN}` (subcollection, not on cursor doc — same 1-MiB-ceiling trap 4e-1-infra solved with mlTraining). 13-min watchdog, BATCH_SIZE=50. Terminal-only `writeSnapshot` (proven by checkpoint test) + `pruneOldSnapshots(keep=30)` + partial subcollection cleanup. **W2 de-hang:** `target-board.ts` introduces `SNAPSHOT_ONLY_UNIVERSES = {russell2k, russell, sp500}` — these NEVER inline-scan; stale → serve `source: 'snapshot-stale'` with `stale: true`, missing → `source: 'snapshot-missing'` with empty targets. `FRESHNESS_BUDGETS_MS['target-board']` widened 30 min → 26 h so the nightly snapshot reads "fresh" all day. dow/ndx/core keep the live fallback (small enough). **W3 enrichment:** new `shared/ticker-reference.ts` with `tickerReference/{ticker}` Firestore cache (Polygon `/v3/reference/tickers` on miss, in-repo fallback on Polygon failure). Bulk `enrichTickerNames(survivors)` runs before pass-2; every Target carries `companyName` + `sector` (from `findEntry().sector`, same value sector-rotation uses). **W4 UI:** `TargetCard` + `TargetDetail` render companyName + sector; `FreshnessPill` adds `snapshot-stale` (amber "As of {age}") + `snapshot-missing` (neutral "No scan yet") states. **Scheduling:** scheduled cron rewritten to `0 23 * * *` (7pm ET); old `0,30 13-21 * * 1-5` cron removed; stopgap `scan-target-board-russell2k-nightly.ts` deleted. Tests 694 → 746 (+52 across 6 new files: cursor, ticker-reference, snapshot-store-4h, target-board-snapshot-only, checkpoint chain, enrichment). `tsc --noEmit` + `npm run build` clean. **Live acceptance deferred** to orchestrator post-merge per `reports/phase-4h/verification.md`: fire manual russell2k scan, confirm `invocationCount > 1`, probe `/api/target-board?universe=russell2k` for sub-2-sec latency, inspect companyName/sector on a pick, verify cron registration. APP_VERSION 0.18.4 → 0.18.5-alpha; MODEL_VERSION unchanged. |
