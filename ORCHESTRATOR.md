...
| 4k | Desktop layout / responsive redesign | brief + kickoff ready (defaults locked) | — | — | **Brief `briefs/phase-4k-brief.md`** (CTO+CFO). TradeIQ is mobile-first; on desktop it's the phone layout stretched. W1 responsive foundation + persistent desktop nav (breakpoint-gated — mobile path untouched); W2 master-detail — the stock detail as a docked side panel on desktop instead of a full-screen modal; W3 desktop-density sortable tables for the target + insider boards. Frontend-only — no run cost, no LLM tokens. **Designed to run in parallel with 4m+4n** — file ownership split (4k owns App.jsx + Target/Insider views + layout primitives; 4m+4n owns Williams/Lynch views + backend). Decisions (defaults, 2026-05-18): breakpoint ~1280px, persistent left sidebar, push/resize detail panel. Kickoff `kickoffs/phase-4k-executor.md`. ~3-4h. APP_VERSION → ~0.19.0-alpha. |
