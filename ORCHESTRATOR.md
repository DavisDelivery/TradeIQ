...
| 4k | Desktop layout / responsive redesign | brief ready (awaiting Chad: 3 decisions) | — | — | **Brief `briefs/phase-4k-brief.md`** (CTO+CFO). TradeIQ is mobile-first; on desktop it's the phone layout stretched. W1 responsive foundation + persistent desktop nav (breakpoint-gated — mobile path untouched); W2 master-detail — the stock detail as a docked side panel on desktop instead of a full-screen modal; W3 desktop-density sortable tables for the target + insider boards. Frontend-only — no run cost, no LLM tokens. **Designed to run in parallel with 4m+4n** — file ownership split (4k owns App.jsx + Target/Insider views + layout primitives; 4m+4n owns Williams/Lynch views + backend). 3 open decisions §X: desktop breakpoint, nav style, side-panel behavior. ~3-4h. APP_VERSION → ~0.19.0-alpha. |
...
| 4m | Williams + Lynch signalization — discrete buy/sell signals | brief ready (combined with 4n — `briefs/phase-4m-4n-brief.md`) | — | — | **Combined 4m+4n brief.** Investigation confirmed the Williams + Lynch analysts are legitimate. 4m (W1-W3): convert the continuous score+side into discrete verdicts — **Williams a trade signal** (BUY/SELL/HOLD + entry/stop/target, volatility-based), **Lynch an investment signal** (BUY/HOLD/AVOID + fair-value band + fundamental-invalidation condition — NOT a forced price stop; honest to a GARP strategy); surface in WilliamsView/LynchView. Bumps MODEL_VERSION. |
| 4n | Williams + Lynch backtest validation | brief ready (combined with 4m — `briefs/phase-4m-4n-brief.md`) | — | — | 4n (W4-W5 of the combined brief): add a point-in-time scoring path for the Williams + Lynch boards to the backtest engine (today Prophet-only), run the discrete signals, produce win-rate/return/drawdown verdict reports. **Critical integrity point — the Lynch backtest must use point-in-time fundamentals or it has look-ahead bias; a biased backtest that prints a flattering number is a negative deliverable.** Brief is explicit: ship 4m first if 4n's PIT work proves deep. Large combined phase (~5-7h); runs parallel with 4k. 3 open decisions §X. |
