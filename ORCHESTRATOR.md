...
- **Phase 4e — Prophet Portfolio** (no brief yet). Chad's product direction 2026-05-13: live self-managing 10-stock portfolio benchmarked against SPY, with an ML loop to improve allocation over time. Depends on 5a findings to choose the ranking signal. Hard product test: must beat SPY or there's no point shipping.

Recommended next: 5a — its findings determine whether 4e uses the composite or an ML output as the picking signal.
