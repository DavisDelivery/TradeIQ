...
| 4l | Insider board completeness | PR open · ready-for-review · awaiting orchestrator live verification | 0.18.5-alpha | 0.18.6-alpha | **Surfaced 2026-05-17 from Chad's observation** that the insiders-purchasing tab only shows ~33 tickers. Confirmed by live probe: two defects. **A —** `/api/insider-board?index=all` skips snapshots entirely and runs a capped live scan (`LIVE_SCAN_CAP = 80`) — 80 of ~2,245 names. **B —** `scan-insider-russell2k.ts` (~2,000 Finnhub calls) hits the same 15-min Netlify ceiling as 4h and never completes. **Shipped 2026-05-18** (branch `phase-4l-insider-completeness`, ~31 new tests, 777 passing across 83 files). W1 — `insider-board.ts` `index=all` path now reads the four per-universe insider snapshots in parallel, unions them, de-duplicates by ticker (freshest contributor wins), re-windows via `filterRowsToWindow`, sorts, trims; returns `source: 'snapshot-aggregate'`, `generatedAt` of oldest contributor (honest freshness), `partial`/`stale`/`contributingUniverses`/`missingUniverses`/`staleUniverses` flags. Graceful partial when any universe is missing/stale; `force=1` keeps capped live scan as debug escape hatch. W2 — `scan-insider-russell2k.ts` becomes a thin schedule trigger (`30 21 * * 1-5`) POSTing the new `scan-insider-russell2k-background.ts` worker. Worker uses `shared/scan-resume/cursor.ts` + `shared/backtest-resume/{watchdog,reinvoke}.ts` (reused verbatim from 4h): 13-min watchdog, BATCH_SIZE=50, concurrency 8, partial rows in `scanRuns/{runId}/partial/batch-NNNNNN` (under Firestore's 1 MiB ceiling), self-reinvoke via `Context.waitUntil`, terminal-only `writeSnapshot` + atomic `_latest` swap + retention prune. Shared `scan-insider.ts` factored into `runInsiderScanBatch` (used by worker) + `buildRowFromTxs` (shared with `runInsiderScan`); added `enrichPrice` option that attaches Polygon `previousClose` to surviving rows. sp500 measured before migration: 208 tickers ≈ 2-3 min vs. 15-min ceiling — left as single-pass with `enrichPrice: true`. `InsiderBoardRow.price: number \| null` added. W3 — `InsiderBoardView.jsx` defaults to **net buyers** (`netDollars > 0`, sorted `netDollars` desc), with Buyers/Sellers/All toggle (persisted to `?insiderView=`), Sellers re-anchors sort to `sellDollars` desc. New sortable Price column (Polygon close) using project-standard `useSortable`+`SortableTh`. Empty-state copy adapts to active view. APP_VERSION 0.18.5-alpha → 0.18.6-alpha. **Orchestrator post-merge:** fire Russell insider scan, confirm `insiderScanRuns/{runId} status: done` + `invocationCount > 1`; probe `/api/insider-board?index=all` for `source: 'snapshot-aggregate'`, materially-more-than-33 names, <2s latency. PR: `briefs/phase-4l-pr-description.md`, verification: `reports/phase-4l/verification.md`. |
