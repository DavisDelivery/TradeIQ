...
| 4j | Detail panel enrichment — company description + price chart | PR open — ready for review | 0.18.6-alpha | 2026-05-17 | All four workstreams shipped on `claude/tradeiq-phase-4j-3F17K`. W1 `ticker-reference.ts` extended to full company info (description, homepage, logo, employees, marketCap, listDate, industry) with `schemaV=2` cache-migration guard so 4h's `{name,fetchedAt}`-only docs are treated as a miss by `getTickerInfo` and refetched lazily on first detail-panel open. W2 `/api/ticker-info` on-demand endpoint + structural guardrail test asserting no `description` field on `Target` / `scan-target.ts` / `analyst-runner.ts` (1 MiB ceiling). W3 `/api/price-history` endpoint with per-ticker-per-range Firestore cache (`merge:true`) refreshed daily. W4 `CompanyInfo` + `PriceChart` (area default, candlestick via recharts custom shape — **no new dependency**) wired into `TargetBoardView.jsx` with 1M/6M/1Y/All toggle (default 6M), responsive on phone + desktop. Verification: tsc clean; 798 tests passing (was 746, +52); build clean. APP_VERSION 0.18.5 → 0.18.6-alpha. See `briefs/phase-4j-pr-description.md` + `reports/phase-4j/verification.md`. Live acceptance deferred to post-merge per the brief. |
