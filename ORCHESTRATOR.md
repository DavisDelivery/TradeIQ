...
| 4h | Russell scan reliability + nightly schedule + company info display | brief + kickoff ready | — | — | Three defects: (1) `scan-target-board-russell2k.ts` can't finish ~2000 names in Netlify's 15-min ceiling; (2) `target-board.ts` hangs ~25s because its stale-snapshot fallback runs a live partial scan **inline in the HTTP request** (root cause corrected from the earlier "Firestore index" guess); (3) no companyName/sector on picks. Full **brief at `briefs/phase-4h-brief.md`** (CTO+CFO: current broken state burns ~105 fn-hr/month ≈ 1,050 Netlify credits for ZERO complete scans; post-4h ~25 fn-hr/month, net ~550-800 credits/month reclaimed + output 0→~30 scans/month). Four workstreams: W1 checkpoint-resume for russell2k+sp500 scans (reuse 4e-1-infra pattern), W2 remove inline-live-scan fallback + widen freshness budget to ~26h, W3 companyName (cached Polygon ticker-reference) + sector (from existing sector-rotation analyst), W4 UI. **Chad's decisions (2026-05-17):** nightly only, 7pm ET (`0 23 * * *`), existing sector labels, keep last 30 snapshots. **Kickoff at `kickoffs/phase-4h-executor.md`** — paste-and-go, ~3-4h agent session. APP_VERSION → 0.18.5-alpha; MODEL_VERSION unchanged. |
