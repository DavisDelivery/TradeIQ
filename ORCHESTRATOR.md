...
| 4j | Detail panel enrichment — company description + price chart | brief + kickoff ready | — | — | From Chad's FLEX detail-panel screenshot — no company description, no price chart. **Brief `briefs/phase-4j-brief.md`**, **kickoff `kickoffs/phase-4j-executor.md`** (paste-and-go, ~2-3h). W1 extend 4h `ticker-reference.ts` to full company info (description/homepage/logo/employees/marketCap/listDate/industry) from the Polygon call it already makes (+ cache-migration guard for 4h's name-only docs); W2 `/api/ticker-info` on-demand endpoint (description NOT on picks — 1 MiB ceiling guardrail); W3 `/api/price-history` endpoint wrapping `getDailyBars`, cached per-day; W4 `CompanyInfo` + `PriceChart.jsx` wired into `TargetBoardView.jsx`. **Chad's decisions (2026-05-17):** chart default 6M with 1M/6M/1Y/All toggle; area chart default + candlestick toggle button (recharts custom shape or `lightweight-charts`); company logos included; all key facts shown; components built responsive for phone AND desktop. Cheap phase — no new background/scheduled functions. APP_VERSION → 0.18.6-alpha. |
