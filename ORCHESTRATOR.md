...
| 4j | Detail panel enrichment — company description + price chart | brief ready (awaiting Chad's 4 decisions) | — | — | **Surfaced 2026-05-17 from Chad's FLEX detail-panel screenshot** — two gaps: the panel shows analyst scores but no company description and no stock price chart. Full **brief at `briefs/phase-4j-brief.md`** (CTO+CFO). Four workstreams: W1 extend 4h's `ticker-reference.ts` to extract description/homepage/employees/marketCap/listDate/industry from the Polygon `/v3/reference/tickers` response it already fetches (+ a cache-migration guard so 4h's `{name}`-only docs get refreshed); W2 new `/api/ticker-info` read endpoint (on-demand — description is NOT enriched onto picks, would risk the 1 MiB snapshot ceiling at russell2k scale); W3 new `/api/price-history` endpoint wrapping the existing `data-provider.ts` `getDailyBars`, cached per-ticker-per-day; W4 `CompanyInfo` block + new `PriceChart.jsx` (recharts area chart, 1M/6M/1Y toggle, mobile-first) wired into `TargetBoardView.jsx`. Cheap phase — no new background/scheduled functions, near-zero incremental run cost (Polygon ticker-details call already paid for by 4h). ~2-3h agent session. APP_VERSION → 0.18.6-alpha. **Open decisions for Chad** (§IX): default chart range, chart type, company logo y/n, which key facts — each with a recommended default. Kickoff goes out once answered. **Out of scope, captured separately:** the earnings analyst's narrow design (it only signals within 21 days of earnings or with ≥4 clean quarters — honestly flags `_noData` otherwise, which is the 4f-finish fix working) and the removed patent analyst (4f-finish zeroed it — Quiver patent coverage too sparse, returned a constant 50) — both are analyst-depth R&D, a larger separate effort. |
