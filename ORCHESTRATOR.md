...
| 4k | Desktop layout / responsive redesign | pending — brief TBD | — | — | **Surfaced 2026-05-17**: Chad will increasingly use TradeIQ on desktop, not just phone, and wants a dedicated desktop-optimized layout designed next. TradeIQ is currently a mobile-first React/Vite SPA — the desktop experience is the mobile layout stretched. A proper desktop version means using the wider viewport: multi-column board views, denser sortable tables, the stock detail as a side/docked panel rather than a full-screen modal, persistent navigation, and charts sized for a real monitor. Needs its own brief (CTO+CFO) once 4j lands. 4j's new components (CompanyInfo, PriceChart) are being built responsive (not mobile-locked) so they carry into 4k cleanly. Sequence: after 4j. |
...
| 4m | Williams + Lynch signalization — discrete buy/sell signals | pending — brief after 4l | — | — | **Surfaced 2026-05-17.** Investigation confirmed the Williams (`styles/williams.ts`) and Lynch (`styles/lynch.ts`) analysts are *legitimate* implementations — Williams genuinely uses Williams %R (14), volatility breakout, TDM seasonality, 20/50-EMA trend gate; Lynch genuinely uses PEG with his thresholds, earnings consistency, the 15-50% revenue sweet spot, debt-to-equity. The real gap: both emit only a continuous `score` (−100..+100) + `side`, not a discrete actionable signal. 4m converts them to clear **BUY / SELL / HOLD** verdicts with entry, stop, and target levels. Belongs as a combined initiative with 4n (define the signals, then validate them). Brief TBD after 4l. |
| 4n | Williams + Lynch backtest validation | pending — brief after 4l (paired with 4m) | — | — | **Surfaced 2026-05-17.** Neither the Williams nor the Lynch signal has ever been backtested — nobody has confirmed a Williams %R turn or a Lynch PEG basket actually produces forward returns. 4n adds a point-in-time scoring path for the Williams and Lynch boards to the backtest engine (today it only supports the Prophet board — `scoreTickerAtDate(...,'prophet',...)`) and runs 4m's discrete signals through it to produce a real win-rate / return / drawdown verdict. Biggest of the three; paired with 4m as one initiative. Depends on 4m (needs defined signals to test) and benefits from 4e-1-infra's checkpoint-resume already being in the backtest engine. Brief TBD after 4l. |
