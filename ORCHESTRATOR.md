...
| 5a-prep | mlTraining row generation — per-candidate emission | brief + kickoff ready | — | — | mlTraining rows are emitted once per held position per rebalance (`engine.ts` ~line 489, `for (const p of target)`); `target` only holds names passing `minComposite >= 50` (~2 per rebalance at sp500 scale), so a full sp500/monthly/7yr backtest yields only ~165 rows vs the ≥10k Phase 5a needs. Chad chose the long-term-correct path: emit one `MLTrainingRow` per **scored candidate** (the full ~500-name universe), with a new `inPortfolio: boolean` field marking held names. Yields ~42,000 rows per sp500/monthly/7yr run and makes the training data cross-sectional (model learns what predicts forward return across the whole universe, not just selection-confirmed picks). **Kickoff at `kickoffs/phase-5a-prep-executor.md`** — paste-and-go, ~2 hour agent session. Touches `engine.ts` + `engine-batched.ts` (both, identically) + `MLTrainingRow` type + equivalence tests; cursor row-count arithmetic must stay correct for batched resume. APP_VERSION → 0.18.4-alpha on merge. Unblocks 5a discovery pipeline. |
