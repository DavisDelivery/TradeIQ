...
| 4l | Insider board completeness | brief + kickoff ready | — | — | **Surfaced 2026-05-17 from Chad's observation** that the insiders-purchasing tab only shows ~33 tickers. Confirmed by live probe: two defects. **A —** `/api/insider-board?index=all` skips snapshots entirely and runs a capped live scan (`LIVE_SCAN_CAP = 80`) — it looks at 80 of ~2,245 names and shows whatever those had; the "all" view is structurally incapable of reflecting the full universe. **B —** the per-universe scheduled insider scans run daily (`30 21 * * 1-5`) and scan their full universe uncapped, but `scan-insider-russell2k.ts` (~2,000 Finnhub calls) hits the same Netlify 15-min ceiling as 4h and never completes. Fix (brief `briefs/phase-4l-brief.md`): W1 — make `index=all` aggregate the four per-universe snapshots (sp500∪ndx∪dow∪russell2k, dedup, re-window) instead of the 80-cap live scan; W2 — apply 4h's checkpoint-resume pattern (`shared/scan-resume/`) to `scan-insider-russell2k.ts` (+ prophylactically sp500). W3 — insider board UI defaults to net buyers with a Buyers/Sellers/All toggle and every column sortable via the standard `useSortable`+`SortableTh` pattern (Chad 2026-05-17). sp500 insider scan: agent measures first, checkpoint-resumes it only if near the ceiling. No LLM-token cost — insider scanning is Finnhub API + math; stays daily. Kickoff `kickoffs/phase-4l-executor.md`, ~2-3h agent session. |
