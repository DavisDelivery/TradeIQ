...
| PR | Title | Version | Why land it |
|---|---|---|---|
| #10 | `hotfix(prophet): import useEffect` | 0.13.2-alpha | **Land first.** Actively crashes prod whenever a user expands a prophet pick row. Sentry firing on it. One-line fix. |
| #9 | Phase 4a hotfix #2: ML-row bar window | 0.13.2-alpha | Phase 5 ML needs the populated `mlTraining` rows; without this PR, every row has `entryPrice: null` and `forward*Return: null`. IC reports as 0.000 because of this. Verified honest-signal IC = -0.0951 with the fix. |

Whichever PR lands second needs a one-line `APP_VERSION` rebase to `0.13.3-alpha` (they both claim 0.13.2). No semantic conflict between the two.
