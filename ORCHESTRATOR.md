...
| 5a | ML training pipeline (discovery) | unblocked; pending re-fire of seed runs (post-0a-2 merge) | — | — | `briefs/phase-5a-brief.md`. Scaffolding shipped (PR #24 OPEN draft — 50 tests, ruff clean, no findings.md). W0 data gate (≥10k rows / ≥5 runs) was blocked on sp500/ndx PIT coverage. Phase 0a-2 (PR open 2026-05-15) closes the sp500 gap; on merge + Netlify redeploy, two sp500 monthly/top50 runs (~8,400 rows) clear the gate. Re-fire after Netlify confirms deploy: same seed configs as before, sp500 universe, 2018-2024 window. NDX coverage still pending (Phase 0a-2b — Invesco QQQ blocked from generator env). Russell2k coverage works back to 2022-01-31. |
