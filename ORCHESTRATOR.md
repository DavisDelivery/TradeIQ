...
| 4i | Portfolio strategy reframing — verdict criteria mismatch | pending (no brief yet) | — | — | **Surfaced 2026-05-16 from the first successful 4e-1-finish full-window backtest** (runId `pb-full-202605160103-5cs65b`, completed cleanly in 21min 38sec across 53 chained invocations under 4e-1-infra). Result: +423.71% portfolio return vs +155.53% SPY (+268% excess) over 2018-2026, but **swapCount=1 over 418 weekly rebalances** — the portfolio essentially never rebalanced. Diagnosis: the PortfolioConfig defaults are explicitly tuned low-turnover (positionCount=10, minHoldDays=30, maxSwapsPerRebalance=3, candidatePool=15). With weekly rebalances + 30-day min-hold, most rebalances can't swap; when they can, only 3 are allowed; and a position must fall from top-10 (where it's held) to outside top-15 to trigger eviction — a huge rank drop that rarely happens for stable large-caps. **The verdict the backtest produces is "Prophet's 2018-01-01 top-10 picks held for 8 years," not "Prophet active-management value."** This is a different and weaker claim than the original 4e-1-finish brief was designed to test. **Decisions needed before shipping any rebalance scheduled function**: (a) re-spec PortfolioConfig defaults if Chad wants active management (raise maxSwapsPerRebalance, lower minHoldDays, widen candidatePool); OR (b) accept the low-turnover framing and ship "Prophet Buy-and-Hold 2018-Top-10" as the actual deliverable with appropriate labeling; OR (c) the verdict is invalid and 4e-1 needs to be re-specced entirely. Direction TBD by Chad. **Blocks 4e-1-finish ship decision.** |
| 5a-prep | mlTraining row generation re-spec — composite filter starves training data | pending (no brief yet) | — | — | **Surfaced 2026-05-16 from the in-flight 0a-2 acceptance backtest** (runId `bt_20260516010323_xanxpf`). Telling numbers at rebalance 40/84: `tickerAttemptTotal: 27992`, `tickerFailureTotal: 0`, `mlTrainingCount: 83`. So 27,992 ticker scoring attempts succeeded with zero PIT failures, but only ~2 names per rebalance survive `minComposite >= 50` and land in `target` — yielding ~83 ml rows after 40 of 84 rebalances. mlTraining rows are emitted once per target position per rebalance (see `engine.ts:518`). The root cause is Prophet's PIT composite scores being low for most historical candidates, likely correlated with the audit finding that **Prophet × largecap catalyst layer has 93% pctFailing** — most candidates can't clear catalyst's pass threshold, dragging composites below 50. **5a's data gate of ≥10k mlTraining rows is unreachable at `minComposite=50`.** Three paths: (1) lower `minComposite` to ~30 for 5a's data-gate runs (more candidates pass, more ml rows, dilutes pick quality but enables a trainable dataset); (2) change ml row generation to emit per-candidate (universe × rebalance), giving ~500 × 84 = 42,000 rows per sp500 monthly/7yr run, which also enables cross-sectional learning rather than just "stocks we'd actually buy"; (3) re-spec 5a's data gate to match Prophet's actual selectivity (~200 ml rows per 2-year run is the realistic budget at current minComposite). Recommend (2) for the long-term — cross-sectional training data is more valuable for ML discovery than selection-confirmed picks anyway. (1) is a quick stopgap if Chad wants to unblock 5a's first run cheaply. **Blocks 5a discovery pipeline ship.** |
