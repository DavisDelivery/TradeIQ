...
| 4o | Russell 2000 scan reliability, round 2 | W1 + W3 done · W2 diagnostic shipped; root cause iterates post-merge | 0.18.8-alpha | 2026-05-18 | **W1 (Bug A fix)** — token-bucket rate limiter in `shared/rate-limiter.ts` (default 55 calls/min via `FINNHUB_RPM` env); `getFinnhubInsiderTransactions` now retries on 429 (3 attempts, exp backoff capped at 8s, `Retry-After` honored) and new `getFinnhubInsiderTransactionsWithStatus` surfaces `{ rateLimited, rateLimitExhausted, errorMessage }` so the scan can propagate failures; russell2k insider concurrency 8 → 4; insider crons staggered (21:30/21:35/21:40/21:45 UTC for russell2k/sp500/ndx/dow). **Finnhub math:** 2,037 / 55 rpm ≈ 37 min Finnhub wall-clock → 3 checkpoint invocations → fits nightly window comfortably on the free tier; no plan upgrade decision required at default. **W3 (systemic)** — new `assessSnapshotPublish` in `snapshot-store.ts` returns `publish | publish-degraded | skip`; both russell2k bg-workers consult it on terminal batch — empty rows on a large universe SKIPS the `_latest` swap (Bug A pattern blocked), ≥50% failure rate skips, ≥10% failure rate publishes with `degraded: true` + reason; rate-limit/error counts propagate into `snapshot.warnings`. **W2 (Bug B — diagnostic shipped, fix iterates)** — per kickoff, NO guessed fix. New `GET /api/scan-status?board=&universe=` exposes cursor (`nextTickerIndex`, `invocationCount`, `lastReinvokeAt`, `reinvokeAttempts`, `invocationAgeMs`); both workers stamp `lastReinvokeAt` BEFORE dispatching self-reinvoke so a stalled chain leaves a forensic trace. Diagnostic hand-off in `reports/phase-4o/verification.md` § "How to use the diagnostic post-merge". 880 tests (was 842 baseline; +38 across 5 new files). |
