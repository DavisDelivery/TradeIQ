...
| 4o | Russell 2000 scan reliability, round 2 | brief + kickoff ready | — | — | **Surfaced 2026-05-18 in 4l post-merge verification.** Both russell2k scans are broken, two different root causes. **Bug A (confirmed) — insider russell2k:** ~2,000+ Finnhub calls at concurrency 8 with no pacing → Finnhub 429 rate-limiting → `getFinnhubInsiderTransactions` swallows every 429 into `[]` (only a `console.warn`) → terminal snapshot published empty (`universeChecked: 2037, results: []`, `warnings: null`) — a silent failure. **Bug B (needs diagnosis) — target-board russell2k (Phase 4h):** never writes a terminal snapshot; board serves a >24h-stale snapshot, companyName/sector null; suspected self-reinvoke handoff stall. Brief `briefs/phase-4o-brief.md` (CTO+CFO): W1 rate-limit-aware Finnhub access (token-bucket throttle, lower concurrency, 429 backoff-and-retry, stagger the 4 insider crons); W2 instrument + diagnose + fix the target-scan stall (debug endpoint exposing the scan cursor is a deliverable); W3 systemic guard — a degraded/empty scan must NOT atomic-swap over a good snapshot, and rate-limit/error counts must propagate into `warnings`. No LLM-token cost. One open decision for Chad (§X): Finnhub plan tier — agent computes whether a throttled russell2k scan completes nightly; upgrade-or-reduce-cadence only if it can't. Kickoff `kickoffs/phase-4o-executor.md` (paste-and-go, ~3-4h). Highest-priority repair on the board. |
