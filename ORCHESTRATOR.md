...
| 4m | Williams + Lynch signalization — discrete buy/sell signals | brief + kickoff ready (combined w/ 4n) | — | — | **Combined 4m+4n brief.** Investigation confirmed the Williams + Lynch analysts are legitimate. 4m (W1-W3): convert the continuous score+side into discrete verdicts — **Williams a trade signal** (BUY/SELL/HOLD + entry/stop/target, volatility-based), **Lynch an investment signal** (BUY/HOLD/AVOID + fair-value band + fundamental-invalidation condition — NOT a forced price stop; honest to a GARP strategy); surface in WilliamsView/LynchView. Bumps MODEL_VERSION. |
| 4n | Williams + Lynch backtest validation | brief + kickoff ready (combined w/ 4m) | — | — | 4n (W4-W5 of the combined brief): add a point-in-time scoring path for the Williams + Lynch boards to the backtest engine (today Prophet-only), run the discrete signals, produce win-rate/return/drawdown verdict reports. **Critical integrity point — the Lynch backtest must use point-in-time fundamentals or it has look-ahead bias; a biased backtest that prints a flattering number is a negative deliverable.** Brief is explicit: ship 4m first if 4n's PIT work proves deep. Large combined phase (~5-7h); runs parallel with 4k. Decisions (defaults, 2026-05-18): Williams ATR-based stop/target, Lynch investment-style signal, backtest = Prophet window / S&P 500. Kickoff `kickoffs/phase-4m-4n-executor.md`. |
