...
| 4e | Prophet Portfolio (self-managing 10-name portfolio vs SPY) | pending (no brief yet) | — | — | Chad's product direction 2026-05-13: live self-managing 10-stock portfolio that uses the Prophet scoring engine to rebalance in/out, tracked against SPY total return, with an ML/RL loop to improve allocation decisions over time. Blocks: needs Phase 5a findings to pick the ranking signal (composite vs ML output), needs portfolio state engine (positions, cash, dividends, slippage modeled), needs scheduled "manager" function that decides swaps on a cadence (probably weekly given turnover/cost), and a new ProphetPortfolioView tab with equity curve vs SPY + holdings table + swap log. Hard product test: "if it can't beat SPY, what's the point" — must ship with the benchmark comparison as the primary metric, not a footnote. |
...
| 5a | ML training pipeline (discovery) | scaffolding only — BLOCKED on data | — | — | `briefs/phase-5a-brief.md` + `briefs/phase-5a-schema-notes.md`. Full pipeline shipped (export, features A/B/C/D, targets, purged walk-forward CV with 5 mandatory tests, models 0–5, metrics + Wilcoxon + Bonferroni, regime analysis, SHAP/perm/PDP, orchestrator producing findings.md). 50 tests passing, ruff clean. **Findings report NOT generated** because Firestore probe (2026-05-14) returned only 389 mlTraining rows across 3 complete runs vs the brief's 10k-row / 5-run minimum. W1 schema notes also documented two material brief vs engine discrepancies: (a) mlTraining rows written only for top-N portfolio picks, so any winning model is structurally a re-ranker not a replacement; (b) field name diffs (`composite` not `compositeScore`, `layers` not `layerScores`, four forward-return horizons not one, no `holdDays`/`inPortfolio`). Awaiting Chad to launch the 5 brief-suggested seed configs via the 4b-2 launcher; pipeline will produce findings.md on next run once data crosses threshold. |
