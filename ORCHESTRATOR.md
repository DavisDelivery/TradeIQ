...
| 4p | Russell 2000 scan terminal-step fix | brief + kickoff ready | — | — | **Diagnosed 2026-05-18 via 4o's `/api/scan-status`.** Both russell2k scans (target-board + insider) successfully walk the entire 2037-ticker universe — reinvoke chain works, cursor reaches the end, ~2022 scored, 41 partial batches accumulate — but the **terminal step** (assemble partial batches → `writeSnapshot` → mark `done`) never completes; the run freezes at `status: running, nextTickerIndex: 2037` permanently (one run stuck since 2026-05-17 23:14). Mechanism: the terminal assemble+write is crammed into the leftover minutes of the last batch-processing invocation and runs out of the 15-min platform budget. **One root cause, breaks both boards** (shared checkpoint-resume machinery). Brief `briefs/phase-4p-brief.md` (CTO+CFO): W1 give the terminal step its own dedicated reinvocation with a fresh 15-min budget (`finalizing` cursor phase) — fix BOTH russell2k workers; W2 make the terminal step idempotent + size-safe (1 MiB ceiling); W3 stuck-`running` run recovery + fix the `/api/scan-status` `scan` param (currently ignored — always returns target-board). No LLM-token cost; net compute *saving* (today's runs waste ~41 min and publish nothing). No open decisions. Kickoff `kickoffs/phase-4p-executor.md` (paste-and-go, ~2-3h). APP_VERSION → ~0.18.9-alpha. |
