...
| 4e-1-finish | Backtest live-data run + verdict | verdicts complete — ship decision pending Chad | 0.18.3-alpha | 2026-05-16 | Four runs total: two dead pre-fix (`pb-full-202605150933-fqrsid` stuck pending; `pb-full-202605151418-8v4k66` killed at 15-min ceiling), then two clean completions under 4e-1-infra. **v1 verdict** (`pb-full-202605160103-5cs65b`, buy-and-hold config): +423.71% vs SPY +155.53% (+268% excess), but swapCount=1/418 — concentration luck, not active signal → triggered Phase 4i. **v2 verdict** (`pb-full-202605161946-osiwpg`, active weekly rebalance config post-4i): +236.46% vs SPY +155.53% (**+80.93% excess**), swapCount 418/418, turnover 8.76%/rebalance (~2 names/wk), maxDD **-27.43%** (better than SPY's -34.10%), Sharpe 0.6603 vs SPY 0.5808, costDrag 0.07%, 53 invocations, 31 min wall-clock. **Honest read**: v2 active rebalance is a legitimate, robust strategy — beats SPY by 81% over 8 years with *lower* drawdown than the index, and doesn't depend on one lucky 2018 pick. It does NOT beat v1's buy-and-hold raw return (concentration luck won on return, lost on risk). **Ship decision is now a genuine product call for Chad**: SHIP v2 as the live `scan-prophet-portfolio-rebalance.ts` scheduled function (defensible — index-beating, controlled drawdown, repeatable), or hold. If SHIP → follow-up PR lands the scheduled function (W5 spec in brief), APP_VERSION bump. |
