...
| 4a-fix-4 | Production hotfix: scheduled-functions deployment + flat layout (PR #12) | done | 0.13.4-alpha | 2026-05-11 | Root cause as diagnosed: Netlify's function bundler silently dropped files in `netlify/functions/scheduled/` subdirectory; zero scheduled scan functions ever deployed to production despite the cron entries in `netlify.toml`. Fix shipped in 3 PRs: (#12) moved all 7 files from `netlify/functions/scheduled/scan-*.ts` → flat `netlify/functions/scan-*.ts`, switched handlers from `Handler` typed export to `schedule(CRON, async () => {...})` wrapper from `@netlify/functions`, removed stale `[functions."scheduled/scan-*"]` blocks from `netlify.toml`, fixed relative imports `../shared/` → `./shared/`; (#13) split 6 multi-universe scans into 23 per-universe functions (`scan-{board}-{universe}.ts`) to fit Netlify's 15-min background-function cap per universe — earnings left monolithic intentionally because its calendar-driven structure scans 320 tickers regardless of universe; (#14) added `seed-scan-background.ts` HTTP-invokable seeder that uses the `-background.ts` filename suffix to get the 15-min container even when called via HTTP (bypasses the 211s gateway timeout that was killing russell2k seeds). 24 scan-* functions now registered on prod with schedules attached. 290 tests passing. After merge + cron fire, every board flipped from `source: fallback-partial` to `source: snapshot`. |
| 4b-1 | Backtest run viewer UI (read-only) | done | 0.14.0-alpha | 2026-05-12 | Read-only viewer for Phase 4a engine runs. New endpoints: `/api/backtest-runs` (sorted list with summary metrics) + `/api/backtest-runs/:runId` (full doc + dailyEquity/trades/attribution/mlTrainingCount subcollections). New hooks: `useBacktestRuns` (30s staleTime) + `useBacktestRun` (Infinity staleTime since runs are immutable; disabled until runId selected). BacktestView fully rewritten (legacy file replaced — engine-test backtest hook stays for EngineTestView). 8 new mobile-first components in `src/components/`: shared `KpiCard` + `ChartPanel` primitives, `SurvivorshipBanner` (gates `corrected:false`, non-dismissible, links to BACKTEST_LIMITATIONS.md), `RunMetricsTiles` (8-tile 2/4-col grid with sign-aware colors), `EquityCurveChart` (LineChart with optional benchmark overlay + 500-point downsample), `DrawdownChart` (AreaChart, computes underwater % if engine didn't pre-compute), `AttributionChart` (per-analyst weighted P&L bars, sign-colored), `RegimeBreakdownTable` + `TopTradesTable` (sortable via useSortable + SortableTh per standing rule; top trades capped at 10 by |P&L%| so winners and losers both surface). LauncherPlaceholder documents prophet-only constraint for 4b-2. 311 tests passing (+21 from 290). Bundle 255.38 kB gzipped (+2.23 kB for all of 4b — well under 820 kB budget). |
| 4b-2 | Backtest run launcher | pending | — | — | UI launcher form for backtest runs. Background-function path so 5-15 min runs survive the 211s HTTP gateway timeout (same architectural decision as `seed-scan-background.ts`). Progress polling for in-flight runs. Prophet-only initially (other boards' point-in-time scoring landed partially in Phase 4a — see BACKTEST_LIMITATIONS.md). Blocked on no other phase; can start whenever. |
