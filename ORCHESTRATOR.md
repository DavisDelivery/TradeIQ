...
Recommended order: **4f next (paste-and-go) + 4e-1-finish (in flight; just wait for completion) + 0a-2 (executor session whenever Chad has Databento creds ready)**. After 4f + 0a-2 land, resume 5a.
...
| 0a-2 | PIT universe-history backfill (sp500 → ndx → russell2k) | pending (brief ready) | — | — | Discovered 2026-05-14 trying to clear Phase 5a's seed-run data gate. The `universeHistory/{universe}/{date}` collection only has live coverage starting ~2026-05-07; historical backtests with 2018-2024 windows on sp500 or ndx complete in 5 seconds with 0 trades because every rebalance date emits "universe pool empty (no PIT snapshot covers date)". Probe of russell2k ran 36+ min and never completed (treated as dead). Only `dow` has working historical PIT coverage. **Brief at `briefs/phase-0a-2-brief.md`** specifies the Wikipedia historical event ingestion + symbol-activity verifier (Polygon adapter built-in; Databento adapter as optional second source — Chad has account) + backward-walk membership reconstruction + survivorship verification. Days of work; unblocks every backtest that needs sp500 history, including Phase 5a (data gate would then clear with ~2 sp500 runs) and any retrospective Prophet/Target board scoring against historical regimes. ndx + russell2k backfill as follow-up sub-phases (0a-2b, 0a-2c). |
...
| 4f | Stub-analyst audit + repair + institutional-flow data (partial) | done (partial; audit PENDING live-data) | 0.18.0-alpha | 2026-05-15 | PR shipped: (a) W1 audit infrastructure — `GET /api/audit-stub-analysts` synchronous endpoint + Sunday-19:00 UTC `scan-stub-audit-cron.ts` covers all 4 quadrants (Target + Prophet × largecap + russell2k); writes to `stubAudits/runs/{stamp}`. CLI mirror at `scripts/audit-stub-analysts.ts`. (b) W4 institutional-flow modules — `shared/institutional-flow/{dark-pool,options-unusual,block-trades,polygon-trades,types}.ts` with 29 tests. (c) W7 weekday-22:00 UTC `scan-institutional-flow-largecap.ts` computes dark-pool + block-trades per largecap ticker (options-unusual shipped + tested but not wired into the scan; follow-up). (d) W3+W5 partial — `shared/compose-weights.ts` + `analyst-runner.ts` repair: insider/patent/political analysts now emit `_noData: true` when activity is null instead of `score: 50`; composite math rescales survivor weights to sum to 1.0; `Target.scoredAnalysts` + `Target.noDataAnalysts` provenance fields added. MODEL_VERSION → `2026.03.0`. Tests 522 → 571 (+49). **W3 further repairs (macro/earnings + threshold_misconfig/latency cases), W5 UI badges, W6 backtest comparison, and options-unusual wiring all deferred to 4f-finish follow-up.** Audit verdict is PENDING LIVE-DATA RUN in `reports/phase-4f/audit.md` — populates itself after merge via the Sunday cron. |
| 4f-finish | Live audit results + remaining W3 repairs + UI badges + W6 backtest | pending | — | — | Triggered by the first Sunday cron after 4f merges. (1) Read `stubAudits/runs/{latest}` to see which Target/Prophet analysts are classified `stub` or `degraded` and their root causes. (2) Repair the `threshold_misconfig`/`latency` cases per kickoff §4.2 taxonomy. (3) Set `ANALYST_WEIGHTS[name] = 0` for any `no_upstream` cases (composeWeights handles the rescale). (4) Ship `LIVE` / `NO_DATA` / `REMOVED` badges in `AnalystContributions.jsx`. (5) Wire options-unusual into `scan-institutional-flow-largecap.ts` once a Polygon options-tick fetcher is built. (6) Run W6 backtest comparison once 4e-1-finish data lands. |
| 5a | ML training pipeline (discovery) | blocked on data gate | — | — | `briefs/phase-5a-brief.md`. Scaffolding shipped (PR #24 OPEN draft — 50 tests, ruff clean, no findings.md). W0 data gate (≥10k rows / ≥5 runs) cannot be cleared in current state: sp500/ndx PIT coverage missing for historical dates, russell2k probe ran 36+ min without completing (treated dead), Dow's 30-name universe caps at ~183 rows/run and cannot reach 10k via more configs alone. Unblock path: Phase 0a-2 (sp500 PIT backfill) → 2 sp500 runs would clear the gate. Alternative: defer 5a, let scheduled scans accumulate over months. Alternative #2: rescope 5a to Dow-pilot with explicit "preliminary signal" framing rather than statistical-claim findings — would need a brief revision. 2026-05-14 recommendation: shelve 5a, prioritize Phase 4f (unblocked, doesn't need historical backtest data), revisit 5a after 4f lands and either 0a-2 lands or several weeks of natural data accumulation have passed. |
