...
| 4e-1-infra | Backtest checkpoint-and-resume (Netlify 15-min Background Function ceiling) | pending (no brief yet) | — | — | **Discovered 2026-05-15** while trying to land the 4e-1-finish full-window verdict. Both `run-portfolio-backtest-background.ts` and `run-backtest-background.ts` are monolithic single-pass implementations with no `waitUntil` / self-reinvoke / cursor checkpointing. Netlify Background Functions cap at 15 minutes wall-clock execution; functions exceeding that are killed silently and the doc remains in `status: running` forever (nothing writes terminal state). Evidence: `pb-short-demo-202605151412-89zcqq` (3-month / 14 rebalances) completed in 14m 45s — right at the wire. `pb-full-202605151418-8v4k66` started 14:18:47 UTC, advanced to running 14:51:34 UTC, never updated again — killed at ~15:06:34 UTC after only 15 minutes of compute. Per-rebalance cost at sp500/monthly = ~63 seconds. Full 7-year window = 84 rebalances × 63 sec = ~88 min — physically impossible single-pass. Fix paths: (a) checkpoint-and-resume via Firestore-stored cursor + `Context.waitUntil()` self-reinvoke loop processing N rebalances per invocation (proven architecture from MarginIQ migration phase M4); (b) function decomposition into a coordinator that dispatches N child invocations and aggregates; (c) external execution environment (Cloud Run, dedicated container) for the heavy work. Recommend (a) — same pattern Chad's team already understands. **Blocks**: 4e-1-finish verdict (needs full 7-year window for statistical significance), 4g (W6 IC comparison + threshold retune validation both need backtests >15 min compute), 5a path-A re-validation if sp500 monthly/top50 ever shrinks below 5 yrs. Workaround until fix lands: shorter windows (1-year max at sp500 monthly) for any acceptance-style test. |
